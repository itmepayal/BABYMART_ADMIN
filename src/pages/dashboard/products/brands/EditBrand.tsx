...
const EditBrand = () => {
  ...
  useEffect(() => {
    ...
    setValue("category", selectedBrand.category ?? "");
    ...
    setValue("isVerified", selectedBrand.isVerified ?? false);
    setValue("sortOrder", String(selectedBrand.sortOrder ?? ""));
    setValue("seoTitle", selectedBrand.seoTitle ?? "");
    setValue("seoDescription", selectedBrand.seoDescription ?? "");

    if (selectedBrand.logo?.url) {
      setValue("logo", selectedBrand.logo.url);
    }

    if (selectedBrand.banners?.length) {
      const urls = selectedBrand.banners.map((b: any) => b.url);
      const ids = selectedBrand.banners.map((b: any) => b.public_id ?? b._id);
      setExistingBannerUrls(urls);
      setExistingBannerIds(ids);
      setBannerPreviews(urls);
      setBannerFiles(new Array(urls.length).fill(null));
    }
  }, [selectedBrand]);
  ...
  const handleBannerUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const remaining = 10 - bannerPreviews.length;
    if (remaining <= 0) {
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      if (logoFile) {
        ...
        formData.append("existingLogo", watchedLogo);
      }

      const keptExistingUrls: string[] = [];
      bannerPreviews.forEach((preview, i) => {
        const file = bannerFiles[i];
        if (file instanceof File) {
          formData.append("banners", file);
        } else {
          keptExistingUrls.push(preview);
        }
      });
      formData.append("existingBanners", JSON.stringify(keptExistingUrls));
  ...
  return (
    <div className="min-h-screen">
      <Header
        title="Edit Brand"
        description="Update brand details, media, and settings"
        actionLabel="Back"
                    ...
                    {bannerPreviews.map((src, i) => (
                      ...
                      >
                        ...
                        <div className="absolute bottom-0 left-0 right-0 bg-black/40 px-2 py-1 text-[10px] text-white">
                          {bannerFiles[i] instanceof File
                            ? (bannerFiles[i] as File).name
                            : `Banner ${i + 1}`}
                        </div>
                      </div>
                    ))}
                      ...
                      {bannerPreviews.length > 0 && (
                        <div className="col-span-2 rounded-2xl border border-slate-200 bg-slate-50 p-3.5">
                          ...
                          <p className="mt-1 text-sm font-bold text-slate-900">
                            {bannerPreviews.length} banner
                            {bannerPreviews.length !== 1 ? "s" : ""}
                          </p>
                        </div>
                      )}
                    </div>
